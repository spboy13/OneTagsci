...
def dailyLoggerFunc():
    ...
    def login():
        ...
        # update gui!
        querySelect = "SELECT * FROM students ORDER BY timeInt ASC"
        curs.execute(querySelect)
        dataFromDataBase = curs.fetchall()
    ...
    def printLog():
        ...
        if username == "admin" and password == "1234":
            ...
            querySelect = ("SELECT * FROM students where GradeNSec = 'STEM - 21'")     # Sample select query, may vary depending on purpose
            curs.execute(querySelect)
            dataFromDatabase = curs.fetchall()  
            conn.commit() #test
            #print to file
            
                        # !!!above repeats for other sections
